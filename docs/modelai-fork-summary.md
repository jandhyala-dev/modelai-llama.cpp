...
- PR-5b is the first solver-complete compaction milestone and is the only branch allowed to claim paper-aligned compression:
  ...
  - minimal internal read-only KV accessors must be added in `src/llama-kv-cache.*` so the solver can read live `K`, `V`, layer layout, and sequence positions without creating a public API,
  - compacted-prefix payloads must be solver-populated from the original KV cache,
  - quality must be regression-tested on fixed tolerances (`attention-output cosine >= 0.95`, `continuation-logit cosine >= 0.95`, explicit partition-sum relative error reporting),
