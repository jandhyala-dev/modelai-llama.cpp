...
void rebuild_layers(
        llama_compacted_prefix_store::sequence_state & state,
        const std::vector<llama_compacted_prefix_layer_layout> & layouts,
        const std::vector<uint32_t> & keep_indices) {
    const uint32_t src_n_tokens = state.logical_positions.size();
    const uint32_t dst_n_tokens = keep_indices.size();

    std::vector<llama_compacted_prefix_store::layer_storage> old_layers = std::move(state.layers);
    state.layers.resize(layouts.size());

    for (size_t i = 0; i < layouts.size(); ++i) {
        state.layers[i].layout = layouts[i];
        state.layers[i].configure(dst_n_tokens);

        if (i >= old_layers.size()) {
            continue;
        }

        const auto & old_layer = old_layers[i];
        auto & new_layer = state.layers[i];
...
bool llama_compacted_prefix_store::seq_rm(llama_seq_id seq_id, llama_pos p0, llama_pos p1) {
    ...
    if (seq_id >= 0) {
        ...
        if (next_positions.empty()) {
            ...
            rebuild_layers(state, layouts, keep_indices);
...
void llama_compacted_prefix_store::seq_cp(llama_seq_id seq_id_src, llama_seq_id seq_id_dst, llama_pos p0, llama_pos p1) {
    ...
    dst.clear(false);
    dst.logical_token_count = src.logical_token_count;
    dst.live_suffix_pos0 = src.live_suffix_pos0;
    ...
    std::vector<uint32_t> keep_indices;
    dst.logical_positions.reserve(src.logical_positions.size());
    keep_indices.reserve(src.logical_positions.size());

    for (uint32_t i = 0; i < src.logical_positions.size(); ++i) {
        const llama_pos pos = src.logical_positions[i];
        if (pos_in(pos, p0, p1)) {
            keep_indices.push_back(i);
            dst.logical_positions.push_back(pos);
        }
    }

    if (dst.logical_positions.empty()) {
        return;
    }

    dst.enabled = true;
    dst.layers = src.layers;
    rebuild_layers(dst, layouts, keep_indices);
}
